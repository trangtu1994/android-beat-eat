...
import com.example.beatoreat.framworks.stage.LocalUserDatasource
...
import com.example.beatoreat.prodframework.datasource.ProdStagingUserDatasource
import com.example.beatoreat.stagingframework.datasource.StagingUserDatasource
...
class MainViewModel(val environment: String): ViewModel() {
    ...
    private fun makeDataSource(environment: String): UserDataSoure =
        when(environment) {
            "Prod" -> {ProdStagingUserDatasource()}
            else -> {StagingUserDatasource()}
        }
